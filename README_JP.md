...
<p align="center">
  <sub>以下の DeepWiki / Zread は DBJD-CR 原プロジェクトの資料であり、v1.2.3 より後の Fork 変更は含まれていません。</sub><br>
...
> [!IMPORTANT]
> **このリポジトリは Sisyphbaous-DT-Project が継続保守する Fork 版です。**
>
> 原プロジェクトは [DBJD-CR](https://github.com/DBJD-CR) によって作成され、原リポジトリは [DBJD-CR/astrbot_plugin_proactive_chat](https://github.com/DBJD-CR/astrbot_plugin_proactive_chat) です。この Fork は原プロジェクト `v1.2.3` のコードラインから継続し、Fork `v1.2.4` を保守の基点とし、Fork `v1.3.0` でグループ分割設定を追加しました。現在の `v1.3.1` では、出力強化プラグインによる分割送信、設定の堅牢性、Web 管理画面の認証、ライフサイクル安全性、ログのプライバシーを重点的に修正しています。
>
> 本 Fork は原作者、過去の貢献履歴、AGPL-3.0 ライセンスを保持しており、原作者リポジトリの公式リリースではありません。詳細は [Fork 保守・バージョン説明](docs/fork-maintenance.md) を参照し、Fork 固有の問題は [現在の Issues](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/issues) へ報告してください。

### v1.3.1 の主な修正

- OutputPro 形式の装飾プラグインが `event.send()` で先行分割を送る際、最初の一文がログにだけ残り、チャットへ届かなかった問題を修正しました。
- 自発メッセージを LLM 結果として標準装飾チェーンへ渡し、先行送信、最終分割、停止、クリア、完全抑止を正しく処理します。
- キャッシュ、履歴、未返信カウンタ、グループ沈黙タイマーを実際の物理送信結果に連動させ、部分成功時に全文を再送しないようにしました。
- グループバッチ、スケジュール、文脈、TTS、分割返信に統一検証を追加し、管理 API は不正設定を拒否し、実行時は古い壊れた設定を安全に補正します。
- Web 管理画面の `no-auth` バイパス、古い Token / WebSocket の残存、URL query への Token 露出を修正しました。
- この Fork から匿名テレメトリと遠隔エラー送信を完全に削除し、例外ログ、識別子、テンポラリファイルの境界を強化しました。
- 現行 AstrBot、AstrBot 4.8、OutputPro タイムラインテストで検証済みです。正確なテスト件数は [CHANGELOG](CHANGELOG.md) を参照してください。
...
> [!NOTE]
> 以下の「開発者からのメッセージ」、プロジェクト履歴、QQ グループ、関連リンクは、帰属と歴史的文脈のため DBJD-CR の原プロジェクトから保持しています。Fork のリリースと Issue 管理は Sisyphbaous-DT-Project リポジトリで行います。
...
1. **この Fork を入手**：[Sisyphbaous-DT-Project Fork リポジトリ](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat) からソースアーカイブを取得するか、`v1.3.1` の正式公開後に [Fork Releases](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/releases) からパッケージを入手し、AstrBot WebUI のプラグインページで `ファイルからインストール` を選択してください。AstrBot プラグインマーケットの同名プラグインは DBJD-CR 原版の場合があります。ここに記載した OutputPro、認証、設定、プライバシー修正を利用するには、プラグイン名だけでなくリポジトリ URL と版番号を確認してください。
...
#### 👥 2.1 グループ分割設定 (`group_batches`)

グループごとに沈黙時間やスケジュールを変えたい場合は、`group_batches` にバッチを追加します。各項目はオブジェクトで、`batch_name`、`session_list`、必要に応じて `group_idle_trigger_minutes`、`min_interval_minutes`、`max_interval_minutes`、`quiet_hours`、`max_unanswered_times`、`proactive_prompt` などの上書き値を持ちます。

Schema ファイルでは AstrBot 4.8 でも読み込めるよう、意図的に通常の `list` として定義しています。新しい AstrBot では起動時にテンプレートリストへ変換され、オブジェクト編集画面が表示されます。プラグイン内蔵 Web 管理画面では同じ値を JSON 配列エディタで編集できます。各バッチは文字列ではなくオブジェクトのまま保存してください。
...
空欄の項目は `group_settings` の対応する値を引き継ぎます。複数のバッチに一致する場合は、最初に一致したバッチが使われ、バッチ設定がグローバルなグループ設定より優先されます。
...
`v1.2.0` 以降、本プラグインは **フロントエンド管理コンソール + モジュール化されたバックエンドコア** という構成へ再設計されました。さらにその後の更新で、**通知システム** と、より整理されたドキュメント構成も追加されています。

- **フロントエンド (`admin/`)**：独立した Web 管理画面を提供し、実行状態表示、タスク管理、通知センター、ドキュメント閲覧、設定編集、リアルタイム同期を担います。
- **バックエンドコア (`core/`)**：セッション設定、スケジューリング、メッセージ送信、文脈構築、永続化、通知同期、Web 管理サービスなどを、責務ごとに分割した複数モジュールで構成しています。
...
バックエンドは `ProactiveChatPlugin` を構成の中心とし、セッション解析、設定反映、永続化、スケジューリング、文脈準備、メッセージ送信、Web 管理画面、通知を独立したモジュールへ分割して連携させています。全体方針は **主クラスが共有状態を一括保持し、各モジュールが具体的責務を分担する** というもので、構造が明快で、保守もしやすくなっています。
...
さらにバックエンドには 2 つの補助サブシステムも接続されています。`WebAdminServer` は実行状態、タスク、設定、通知を HTTP + WebSocket でフロントエンドコンソールへ公開し、`NotificationCenter` は告知同期を担当します。これらは **強化型コントロールプレーン** に属する機能であり、仮に一時的な障害が起きても、自発メッセージ主フローを止めることはありません。
...
| :--- | :--- | :--- | :--- |
| **Fork v1.3.1** | ✅ 現在の保守版 | 装飾分割の実送信、設定・認証・ライフサイクル・プライバシー修正 | v4.8.0+、最新版推奨 |
| **Fork v1.3.0** | ⚠️ 置き換え済み | Fork v1.2.4 を基にグループ分割設定を追加 | v4.8.0+ |
| **Fork v1.2.4** | ✅ Fork の基点 | DBJD-CR 原プロジェクト v1.2.3 を基にした保守版 | v4.8.0+ |
...
現在の Fork リリースは本リポジトリの [CHANGELOG](CHANGELOG.md) と [Fork 保守説明](docs/fork-maintenance.md) を参照してください。上流 `v1.2.3` 以前の履歴は [DBJD-CR 原リポジトリ](https://github.com/DBJD-CR/astrbot_plugin_proactive_chat) で確認できます。
...
**Q: OutputPro などで分割した自発メッセージの最初の一文が表示されません。**

> **A**：この Fork の `v1.3.1` 以上を使用しているか確認してください。旧送信経路の合成イベントは、装飾プラグインによる先行 `event.send()` を物理的に配信できず、最初の分割がログにだけ残る場合がありました。`v1.3.1` では実送信イベントへ変更し、先行分割と最終分割が順番どおり届くことを検証しています。再現する場合は、AstrBot 版、出力強化プラグイン版、匿名化した分割設定、固定エラー番号を報告し、実際の会話本文は含めないでください。

**Q: AstrBot 4.8 ではグループ分割設定を編集しにくいのはなぜですか。**

> **A**：AstrBot 4.8 は新しいオブジェクトテンプレートリスト型 Schema に対応していません。プラグインを正常に読み込めるよう、本 Fork は `group_batches` を通常のリストとして保存します。プラグイン独自の Web 管理画面または設定 JSON を利用してください。新しい AstrBot では対応能力に応じて編集 UI が実行時に拡張されます。

**Q: 匿名テレメトリ設定が見つからないのはなぜですか。**

> **A**：現在の Fork は匿名テレメトリ、遠隔エラー送信、インスタンス ID、ハートビート送信を完全に削除したため、`telemetry_config` は使用されません。通知センターは任意のユーザー向け告知機能として残っていますが、会話本文やエラー詳細はアップロードしません。
...
> **A**：固定エラー番号（例：`PC-SEND-xxx`）、プラグイン版、AstrBot 版、関連プラグイン版、再現手順を記録し、[Fork リポジトリ](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/issues) に Issue を作成してください。実際の会話本文、Prompt、完全な UMO、アカウント識別子、パスワード、Token、未確認の完全スタックトレースは公開せず、必要な設定は必ず匿名化してください。
...
以下の QQ グループと QR コードは上流コミュニティ情報として保持しています。Fork 固有の互換性問題や修正提案は [Fork Issues](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/issues) を優先してください。
...
現在の Fork への [Issue](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/issues) と [Pull Request](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/pulls) を歓迎します。[CONTRIBUTING](CONTRIBUTING.md) を読み、問題が Fork と上流のどちらで発生するかを明記してください。
