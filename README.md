...
### 👥👥 3. 群聊分批配置 (`group_batches`)  **v1.3.0 新增**

如果你管理着多个群聊，并且希望**不同活跃度的群采用不同的主动消息策略**，可以使用群聊分批配置。该功能在 AstrBot 管理面板的插件设置中直接操作，无需修改代码。
...
在 AstrBot 管理面板 → 插件设置 → 主动消息插件中，你会看到 "👥 群聊分批配置" 区域，点击 "+ 添加条目" 即可创建批次：

```json
{
  "group_batches": [
    {
      "__template_key": "group_batch",
      "batch_name": "活跃群（高频）",
      "session_list": [
