...
<p align="center">
  <sub>以下 DeepWiki / Zread 指向 DBJD-CR 原项目资料，不包含当前 Fork 在 v1.2.4 之后的修改。</sub><br>
...
> [!IMPORTANT]
> **这是由 Sisyphbaous-DT-Project 继续维护的 Fork 版本。**
>
> 原项目由 [DBJD-CR](https://github.com/DBJD-CR) 创建，原仓库为 [DBJD-CR/astrbot_plugin_proactive_chat](https://github.com/DBJD-CR/astrbot_plugin_proactive_chat)。当前 Fork 基于原项目 `v1.2.3` 代码线继续维护：Fork `v1.2.4` 是维护起点，Fork `v1.3.0` 加入群聊分批配置，当前 `v1.3.1` 重点修复输出增强插件分段丢失、发送结果误判、配置崩溃、Web 管理端鉴权、生命周期和日志隐私问题。
>
> 本 Fork 保留原作者、历史贡献者和 AGPL-3.0 许可证信息，不代表原作者仓库的官方发布。详细差异请阅读 [Fork 维护与版本说明](docs/fork-maintenance.md)，本 Fork 的问题请提交到 [当前仓库 Issues](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/issues)。

### v1.3.1 修复重点

- 修复 OutputPro 等输出增强插件调用 `event.send()` 分段时，第一段只出现在日志、没有真正送达聊天窗口的问题。
- 主动消息以 LLM 结果进入标准装饰链，支持提前分段、末段回填、停止、清空和纯拦截，不重复发送完整原文。
- 依据真实物理发送结果更新缓存、历史、未回复计数和群聊沉默计时；部分成功不再整句重发。
- 统一校验群聊批次、调度、上下文、TTS 和分段配置；管理端拒绝坏配置，运行时兼容旧坏数据。
- 修复 Web 管理端 `no-auth` 绕过、旧 Token / WebSocket 不失效及 URL 携带 Token 等鉴权问题。
- 移除本 Fork 中全部匿名遥测和远程错误上报，收紧异常日志、会话标识和临时文件清理边界。
- 已在当前 AstrBot、AstrBot 4.8 和 OutputPro 时间线测试中验证，完整测试数字见 [CHANGELOG](CHANGELOG.md)。
...
> [!NOTE]
> 下方“开发者的话”、原项目历史、QQ群和友情链接来自 DBJD-CR 原项目，作为项目历史与致谢保留；当前 Fork 的发布和问题跟踪以 Sisyphbaous-DT-Project 仓库为准。
...
1. **下载当前 Fork**: 从 [Sisyphbaous-DT-Project Fork 仓库](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat) 下载源码归档，或在 `v1.3.1` 正式发布后从 [Fork Releases](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/releases) 获取安装包，然后在 AstrBot WebUI 的插件页面选择 `从文件安装`。AstrBot 插件市场中的同名插件可能来自 DBJD-CR 原项目；若要使用本文所述的 OutputPro、鉴权、配置与隐私修复，请核对仓库地址和版本号，不要只凭插件名称判断。
...
### 👥👥 3. 群聊分批配置 (`group_batches`)  **Fork v1.3.0 新增**

如果你管理着多个群聊，并且希望**不同活跃度的群采用不同的主动消息策略**，可以使用群聊分批配置。配置在底层是 `list[object]`，这样 AstrBot `4.8.0` 也能正常加载插件；较旧的 AstrBot 面板可能只提供通用列表编辑器，此时可以直接编辑插件配置 JSON。新版面板若提供对象列表编辑器，也可以直接在面板中操作。

> **核心规则**
> - 全局 `group_settings.enable` 仍是总开关；关闭后所有批次失效。
> - 未加入任何批次的群聊，继续使用全局 `group_settings` 的配置。
> - 一个群聊同时存在于全局 `session_list` 和某个批次中时，**优先使用批次配置**。
> - 批次中未填写的字段，会自动继承全局群聊配置。
...
- **主动消息提示词 (`proactive_prompt`)**:
  - 类型：`Text`
  - 默认值：空字符串
  - 说明：**留空则继承全局群聊提示词**；填写后将覆盖全局提示词，仅对该批次生效。
...
### 🔔 5. 通知系统设置 (`notification_settings`)
...
后端以 `ProactiveChatPlugin` 为装配中心，把会话解析、配置生效、持久化、调度、上下文准备、消息发送、Web 管理端与通知拆成独立模块协同运行。整体思路是**主类统一持有状态，各模块分工完成具体职责**，因此结构更清晰，也更方便后续维护。
...
除此之外，后端还挂接了两个附属子系统：`WebAdminServer` 负责把运行状态、任务、配置和通知以 HTTP + WebSocket 形式暴露给前端控制台；`NotificationCenter` 负责公告同步。这些能力都属于**增强型控制面**，即使它们偶发失败，也不会阻断主动消息主流程。
...
| :--- | :--- | :--- | :--- |
| **Fork v1.3.1** | ✅ 当前维护版 | 修复装饰分段真实发送、配置与鉴权安全、生命周期和隐私问题 | v4.8.0+，推荐最新版 |
| **Fork v1.3.0** | ⚠️ 已被替代 | 基于 Fork v1.2.4 加入群聊分批配置 | v4.8.0+ |
| **Fork v1.2.4** | ✅ Fork 基线 | 基于 DBJD-CR 原项目 v1.2.3 代码线建立维护版本 | v4.8.0+ |
...
当前 Fork 的详细版本说明见本仓库 [CHANGELOG](CHANGELOG.md) 和 [Fork 维护说明](docs/fork-maintenance.md)；原项目 `v1.2.3` 及更早历史可查看 [DBJD-CR 原仓库](https://github.com/DBJD-CR/astrbot_plugin_proactive_chat)。
...
**Q: 使用 OutputPro / 输出增强分段后，主动消息第一段没有显示？**

> **A**: 请确认使用的是当前 Fork `v1.3.1` 或更高版本。旧发送链中的合成事件无法真正执行装饰插件的 `event.send()`，因此可能出现第一段只写入日志、没有发到聊天窗口的情况。`v1.3.1` 已改为真实发送事件，并验证提前分段与末段会按顺序送达。若仍能复现，请提供 AstrBot 版本、输出增强插件版本、脱敏后的分段配置和固定错误编号，不要粘贴真实聊天正文。

**Q: AstrBot 4.8 中为什么不方便编辑群聊批次？**

> **A**: AstrBot 4.8 的原生配置解析器不支持新版对象模板列表。本 Fork 为了保证插件能正常加载，将磁盘 Schema 保持为普通 `list`。可以使用插件自己的 Web 管理端或直接编辑配置 JSON；新版 AstrBot 会在运行时按能力升级为对象模板编辑器。

**Q: 为什么找不到匿名遥测开关？**

> **A**: 当前 Fork 已完全移除匿名遥测、远程错误上报、实例 ID 和心跳上报，因此 `telemetry_config` 不再生效。通知中心仍然保留，但它只是可选的用户可见公告功能，不会上传聊天内容或错误详情。
...
> **A**: 请记录日志中的固定错误编号（例如 `PC-SEND-xxx`）、插件版本、AstrBot 版本、相关插件版本和可复现步骤，并提交到当前 Fork 的 [Issues](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/issues)。不要公开粘贴真实聊天正文、Prompt、完整 UMO、账号、密码、Token 或未经检查的完整堆栈；必要配置请先脱敏。
...
以下 QQ 群与二维码来自原项目，作为上游社区信息保留。当前 Fork 的兼容性问题、修复建议和代码贡献，请优先使用 [Fork Issues](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/issues)。
...
欢迎向当前 Fork 提交 [Issue](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/issues) 和 [Pull Request](https://github.com/Sisyphbaous-DT-Project/astrbot_plugin_proactive_chat/pulls)。提交前请阅读 [CONTRIBUTING](CONTRIBUTING.md)，并说明问题是在 Fork 版本还是原项目版本中出现。
