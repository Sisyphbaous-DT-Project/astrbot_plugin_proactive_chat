...
class ProactiveCoreMixin:
    ...
    async def _finalize_and_reschedule(
        ...
        try:
            candidates = self._build_sender_conversation_session_candidates(delivery_session_id)
            for candidate in candidates:
                try:
                    current_conv_id = await self.context.conversation_manager.get_curr_conversation_id(candidate)
                    logger.info(
                        f"[主动消息] 主动消息发送后 UMO 映射检查："
                        f"candidate={candidate}, current_conv_id={current_conv_id}, target_conv_id={conv_id}"
                    )
                except Exception as e:
                    logger.warning(f"[主动消息] UMO 映射检查失败：candidate={candidate}, error={e}")
        except Exception as e:
            logger.debug(f"[主动消息] 主动消息发送后 UMO 映射检查整体失败：{e}")
    ...
    async def check_and_chat(self, session_id: str) -> None:
        ...
        try:
            ...
            history_written = await self._persist_proactive_pair_to_conversation_history(
                session_id=delivery_session_id,
                conv_id=conv_id,
                user_prompt=final_user_prompt,
                assistant_response=response_text,
                session_candidates=request_package.get("candidate_session_ids"),
            )

            if not history_written:
                logger.warning(
                    f"[主动消息] 主动消息已发送，但写入 AstrBot LLM 对话历史失败："
                    f"session={delivery_session_id}, conv_id={conv_id}"
                )
