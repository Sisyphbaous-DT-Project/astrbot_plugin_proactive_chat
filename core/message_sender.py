...
class SenderMixin:
    ...
    async def _persist_proactive_pair_to_conversation_history(
        ...
        session_candidates: list[str] | None = None,
    ) -> bool:
        """把主动消息安全写入 AstrBot conversation history，并强制对齐所有 UMO 别名。"""
        ...
        if not candidates:
            for item in self._build_sender_conversation_session_candidates(session_id):
                text = str(item or "").strip()
                if text and text not in candidates:
                    candidates.append(text)
            _append = lambda v: (candidates.append(str(v).strip()) if str(v).strip() and str(v).strip() not in candidates else None)
            _append(session_id)
            try:
                _append(self._normalize_session_id(session_id))
            except Exception:
                pass

        if not candidates:
            logger.warning("[主动消息] 没有可用的 UMO 候选，无法写入主动消息历史。")
            return False

        user_text = "[系统事件：主动消息触发]"
        ...
        try:
            ...
            for candidate in candidates:
                try:
                    if await conv_mgr.get_curr_conversation_id(candidate) == conv_id:
                        effective_session_id = candidate
                        break
                except Exception:
                    continue

            lock_key = effective_session_id
            if session_lock_manager is not None:
                lock_ctx = session_lock_manager.acquire_lock(lock_key)
            else:
            ...
            async with lock_ctx:
                conversation = await self._get_conversation_for_history(
                    conv_mgr, effective_session_id, conv_id
                )
                if not conversation:
                    logger.warning(
                        f"[主动消息] 找不到目标对话，无法写入主动消息历史："
                        f"session={effective_session_id}, conv_id={conv_id}"
                    )
                    return False
                ...
                if len(history) >= 2:
                    ...
                    if (
                        ...
                        and self._extract_history_content_text(last_user.get("content")) == user_text
                        and self._extract_history_content_text(last_assistant.get("content")) == assistant_text
                    ):
                        logger.debug(
            ...
            verify = await conv_mgr.get_conversation(effective_session_id, conv_id)
            verify_len = 0
            verify_last_role = "未知"
            if verify and verify.history:
                try:
                    vh = json.loads(verify.history) if isinstance(verify.history, str) else verify.history
                    if isinstance(vh, list):
                        verify_len = len(vh)
                        if vh and isinstance(vh[-1], dict):
                            verify_last_role = str(vh[-1].get("role") or "未知")
                except Exception:
                    pass

            logger.info(
                f"[主动消息] 已写入主动消息到 AstrBot LLM 对话历史："
                f"effective_session={effective_session_id}, conv_id={conv_id}, "
                f"history_len={verify_len}, last_role={verify_last_role}, candidates={candidates}"
            )
            return True
